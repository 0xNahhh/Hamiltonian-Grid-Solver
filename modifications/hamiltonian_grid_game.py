...
invalid_cell_holes = [[i,j] for i in range(3) for j in range(3)]
def generateCellHoles():
  cell_holes = []
  for _ in range(4):
    hole = [0,0]
    while hole in invalid_cell_holes or hole in cell_holes:
      hole = [random.randrange(12), random.randrange(12)]
    cell_holes.append(hole)

  return cell_holes
print(generateCellHoles())
...
class HamiltonianGrid(gym.Env):
  ...
  def __init__(self, cell_holes = None, n = 12):
    super(HamiltonianGrid, self).__init__()

    self.cell_holes = cell_holes if cell_holes else generateCellHoles()
    self.n = n
  ...
  def step(self, action):
    ...
    if (sum(self.visited) == self.n**2 - 4 or (new_position != self.position and self.visited[new_position] == 1)):
      self.done = True
    ...
    if (not self.done and new_position == self.position):
      self.reward = -0.5
    if (self.done and sum(self.visited) == self.n**2 - 4):
      self.reward = 1000
    if (self.done and sum(self.visited) != self.n**2 - 4):
      self.reward = -2
    ...
    print("cell_holes: ", self.cell_holes)
    print("action: ", action)
  ...
  def reset(self):
    self.cell_holes = generateCellHoles()
